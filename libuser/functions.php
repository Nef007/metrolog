<?php
	...
	function isAdmin($login = false, $password = false) {
		 ...
		 if (!isset($_SESSION["id"]))
			return false;
		 	...
		 	elseif ($_SESSION["id"] == 1) return true;}
		// if (!$id) $id = isset($_SESSION["id"])? $_SESSION["id"] : false;
		// if (!$password) $passwor
		// d = isset($_SESSION["password"])? $_SESSION["password"] : false;
	...
	function getOrders() {
		...
		$query = "SELECT DISTINCT `id`,`dev_name`,`dev_marka`,`dev_zav_number`, `dev_data_pred_poverki`, `dev_data_release`,`dev_data_poverki` FROM `device`, `users` WHERE users.distr_id={$_SESSION['distr_id']} and users.distr_id=device.dist_id";
